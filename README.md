...
- 모델의 레이어 몇 개를 GPU에 올릴지(`-ngl`)
- 대화 맥락을 얼마나 길게
잡을지(컨텍스트 길이)  

조합은 수십 가지인데, 지금은 대부분 하나씩 돌려
보며 감으로 찾습니다.

고르는 사람이 정말 알고 싶은 건 세 가지 질문입니다.  
- 이 기기에서 목표
속도가 나오는가?  
- 품질손실이 몇 %인가?  
- 목표를 만족하는 가장 가벼운
...
| 비교 대상 | ① 목표 속도 | ② 품질손실 | ③ 최소 구성 |
|---|---|---|---|
| VRAM 계산기 | 적재 여부만 추정 | 다루지 않음 | 다루지 않음 |
| 챗봇 조언 | 단순 추정 | 일반론 | "전부 올려라" |
| NVIDIA AutoQuantize | 속도 목표 입력이 없음 | 다룸 — 단 CUDA 전용 | CUDA 전용 |
| **FiTuna** | **실측으로 판정** | **실측으로 판정** | **이진탐색으로 확정** |

FiTuna가 실측을 기반으로 찾은 최적의 구성은 `--export-ollama`로 그대로 Ollama에 넘길 수 있습니다.

단순 추정과 실측에 기반한 결과는 차이가 큽니다. 하단의 **실측 결과**에서 볼 수 있듯이 실제로
"당연히 가장 좋을" Q8_0이 속도 분야에서 탈락했고, **Q4_K_M에 최소 오프로드 `-ngl 33`을 더한 조합**이 가장 목표에 가까운 속도를 보여주었습니다.  
이런 결과는 **추측만 아무리 해서는 절대 알 수 없었습니다.**
