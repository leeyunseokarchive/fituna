...
고르는 사람이 정말 알고 싶은 건 세 가지 질문입니다. **① 이 기기에서 목표
속도가 나오는가? ② 품질손실이 몇 %인가? ③ 목표를 만족하는 가장 가벼운
구성은 무엇인가?** — 그런데 이 질문에 답해 주는 도구가 없었습니다:

| | ① 목표 속도 | ② 품질손실 | ③ 최소 구성 |
|---|---|---|---|
| VRAM 계산기 | 적재 여부만 추정 | 다루지 않음 | 다루지 않음 |
| 챗봇 조언 | 추정 — [실험 3회 모두 실측과 어긋남](docs/CHATBOT_COMPARISON.md) | 일반론 | "전부 올려라" |
| NVIDIA AutoQuantize | 속도 목표 입력이 없음 | 다룸 — 단 CUDA 전용 | CUDA 전용 |
| **FiTuna** | **실측으로 판정** | **실측으로 판정** | **이진탐색으로 확정** |

Ollama나 LM Studio 같은 실행기는 이 표에 없습니다 — 질문에 답하는 도구가
아니라 정해진 구성을 실행하는, 용도가 다른 보완 도구이기 때문입니다.
FiTuna가 찾은 구성은 `--export-ollama`로 그대로 Ollama에 넘길 수 있습니다.
