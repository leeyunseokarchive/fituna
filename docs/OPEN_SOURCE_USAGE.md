...
`llama-cli` is checked by `fituna doctor` (`doctor.py:300`) as an optional
check; `llama-imatrix` is resolved by
`fituna/binaries.py:99` and printed by `fituna list-binaries`
(`cli.py:235`), and no code path invokes it today. Stating this precisely
...
by FiTuna itself — but the user can trigger it, so `cli.py:288`
...
| Attribution + share-alike | CC BY-SA 3.0 corpora | `fituna fetch-corpus` prints the notice and source URL on every fetch (`cli.py:288`); no corpus text is committed |
